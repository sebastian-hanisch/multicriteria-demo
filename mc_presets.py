"""SETTING_SPECS-Permalink-Muster, Presets und Zufalls-Seed-Button (Standardmuster aus dem Demo-Portfolio, siehe jo_presets.py in johnson-demo)."""
...
SETTING_SPECS = {
    ...
    "conflict_slider": SettingSpec("conflict", float, C.DEFAULT_CONFLICT, C.CONFLICT_MIN, C.CONFLICT_MAX),
    ...
    "corr_slider": SettingSpec("corr", float, C.DEFAULT_CORR, C.CORR_MIN, C.CORR_MAX),
    "links_slider": SettingSpec("links", int, C.DEFAULT_LINKS, C.LINKS_MIN, C.LINKS_MAX),
    "mode_select": SettingSpec("mode", _choice(C.MODES), C.DEFAULT_MODE),
    "seed_input": SettingSpec("seed", int, C.DEFAULT_SEED, 0, 2_000_000_000),
}
PRESET_KEYS = {"net": "net_select", "side": "side_slider", "conflict": "conflict_slider", "nodes": "nodes_slider", "degree": "degree_slider", "corr": "corr_slider", "links": "links_slider",
               "mode": "mode_select", "seed": "seed_input"}
...
KEPT = {key: f"_kept_{key}" for key in ("side_slider", "conflict_slider", "nodes_slider", "degree_slider", "corr_slider", "links_slider", "mode_select", "seed_input")}
    ...
    for state_key, spec in SETTING_SPECS.items():
        if spec.url_param in qp:
            try:
                ...
                pass
    for key, step in (("conflict_slider", 0.1), ("corr_slider", 0.25), ("degree_slider", 0.5)):
